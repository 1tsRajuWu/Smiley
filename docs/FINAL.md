# A note from Raj — Smiley's final chapter

**Raj (@1tsRaj / 1tsRajuWu)** — July 9, 2026 · **Smiley v12**

---

Smiley started as a fun personal project — something I built because I wanted Discord Rich Presence with anime GIFs, and I kept polishing it until it felt *near perfect*. That journey was a blast, and I'm grateful to everyone who used it, starred the repo, reported bugs, and shared feedback along the way.

## v12 is the final version

**Smiley v12 is the last major release.** After v12.0.4, expect **only occasional bug fixes** (`v12.0.x`) when something breaks — not a steady stream of new features. A major v13 is **not planned**. Consider this a warm goodbye from active development, not from the project itself.

The **community is completely free** to use, fork, modify, and advance the code. Smiley is MIT-licensed. If you want live gaming presence, music sync, new skins, or anything else in a direction I didn't take — please fork it and make it yours.

**Shipping app:** [`Smiley.v12/`](../Smiley.v12/) · **Detailed v12 note:** [`Smiley.v12/docs/FINAL-V12.md`](../Smiley.v12/docs/FINAL-V12.md)

## What stays open

| Welcome | Author infrastructure only |
|---------|---------------------------|
| Read, fork, and modify the source | Smiley install database (Supabase) |
| Pull requests and community forks | Live install telemetry on my servers |
| Your own Discord app + API keys | Bundled `discord.app.json` / my Client ID |
| Your own donation links | `paypal.me/1tsRaj` in official builds |

See [CONTRIBUTING.md](../CONTRIBUTING.md) for fork setup.

## Thank you

To everyone who downloaded Smiley, left reviews, opened issues, or just had fun showing off on Discord — **thank you**. This was a personal project built for the joy of making something useful and a little silly. I'm glad it helped some of you express what you're up to.

Take care, and happy forking. 👋

— **Raj**

[README](../README.md) · [Contributing](../CONTRIBUTING.md) · [Legal](../LEGAL.md) · [Privacy](../PRIVACY.md)
