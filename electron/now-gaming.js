...
async function pollForeground() {
  if (process.platform === 'darwin') return pollMac();
  if (process.platform === 'win32') return pollWin();
  if (process.platform === 'linux') return pollLinux();
  return null;
}
...
module.exports = { createNowGamingService, pollForeground, normalizeGame, gameSig };
