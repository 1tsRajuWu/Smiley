...
s.initSecurity(ud, null);
const localPayload = { secret: 'settings', favorites: ['a'] };
const encLocal = s.encryptJson(localPayload, ud);
ok('Local encrypt produces envelope', encLocal.v >= 3 && encLocal.iv && encLocal.tag);
...
s.initSecurity(ud2, mockSafeStorage);
ok('Keychain mode active', s.isKeychainActive());
const encKc = s.encryptJson({ x: 1 }, ud2);
ok('Keychain encrypt v4', encKc.v === 4);
ok('Keychain decrypt', s.decryptJson(encKc, ud2).x === 1);
