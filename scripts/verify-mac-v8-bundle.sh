...
APP="$(find "$ROOT/Smiley.v8/src-tauri/target" -path '*/release/bundle/macos/*.app' 2>/dev/null | head -1)"

if [[ -z "$APP" ]]; then
  echo "::error::No macOS .app bundle found under Smiley.v8/src-tauri/target"
  exit 1
fi
