...
impl App {
    ...
    pub fn set_activity(&self, id: &str) -> AppResult<Status> {
        ...
        // User-configured debounce (manual picks). Rotate uses same path — still debounce.
        {
            ...
            if let Some(at) = *last {
                if at.elapsed() < cooldown {
                    // Same id within cooldown: treat as success / no-op so UI doesn't error
...
fn local_minutes() -> u32 {
    ...
    if let Ok(out) = Command::new("date").arg("+%H:%M").output() {
