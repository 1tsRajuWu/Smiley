//! GitHub release check for Smiley v8 tags only.
...
fn parse_v8_version(tag: &str) -> Option<Vec<u32>> {
    ...
    let parts: Vec<u32> = s
        ...
        .collect();
    if parts.first() == Some(&8) && parts.len() >= 2 {
        Some(parts)
    } else {
        None
    }
...
pub fn check_for_updates() -> AppResult<UpdateCheck> {
    ...
    let current_parts = parse_v8_version(&current).unwrap_or_else(|| {
        current
            .split('.')
    ...
    let client = reqwest::blocking::Client::builder()
        ...
        .user_agent(format!("Smiley-v8/{current}"))
        .build()
    ...
    for rel in releases {
        ...
        if let Some(ver) = parse_v8_version(&rel.tag_name) {
            if best
                .as_ref()
    ...
    let Some((latest_parts, latest_ver, html_url)) = best else {
        return Ok(UpdateCheck {
            ...
            releases_url,
            download_url: None,
            message: "No v8 release found on GitHub yet.".into(),
        });
    };
...
mod tests {
    ...
    fn parses_v8_tags() {
        assert_eq!(
            parse_v8_version("v8.0.3"),
            Some(vec![8, 0, 3])
        );
        assert!(parse_v8_version("v7.9.26").is_none());
    }

    #[test]
    fn compares_versions() {
        assert_eq!(
            cmp_versions(&[8, 0, 3], &[8, 0, 2]),
    ...
    fn allows_github_release_urls() {
        assert!(is_safe_release_url(
            "https://github.com/1tsRajuWu/Smiley/releases/tag/v8.0.3"
        ));
